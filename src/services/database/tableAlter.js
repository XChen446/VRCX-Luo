...
const tableAlter = {
    ...
    async updateTableForGroupNames() {
        ...
        if (!tables.includes('gamelog_location'))
            tables.push('gamelog_location');
        for (var tableName of tables) {
            try {
                await adapter.alterTableAddColumn(
                    tableName,
        ...
        try {
            await adapter.alterTableDropColumn('gamelog_location', 'groupName');
        } catch (e) {
            e = e.toString();
