...
// 调试开关: true=双路径并行运行并对比结果, false=只运行一条路径
// 验证完成后可删除
const USE_DUAL_PATH_DEBUG = false;
...
export const useVrcxStore = defineStore('Vrcx', () => {
    ...
    async function runFixes() {
        if (USE_NEW_MIGRATION) {
            ...
            await database.runMigrations(state.databaseVersion, TARGET_DB_VERSION);
        } else {
            // 旧路径: 调用遗留的 fix 函数
