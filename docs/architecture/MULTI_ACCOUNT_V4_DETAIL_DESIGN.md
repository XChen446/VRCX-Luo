# VRCX-K 多账号支持 · V4 详细设计文档

> **方案**: 单进程 · 单内存池 · 中间层拦截 (Plan B/方案三)
> **状态**: 已实现（2026-08 随 database-refactor 并入 main）——本文档为**历史设计稿**，部分章节与当前实现存在偏离（如次号 Cookie 持久化、UNION ALL 聚合查询、通知合并等未按设计落地），仅作决策记录与架构参考，不作现状文档使用。
> **最后更新**: 2026-04-30（设计稿）
