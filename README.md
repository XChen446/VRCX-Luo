...
> 你知道的，看到提交历史就应该知道 ~~这个分支干的不是什么光彩的事 (bushi)<br>~~
> 这个分支做的最主要的事就是强制去除了自动加入下文中自动加入`VRCX-Luo`的Home Group 的功能<br>
> 对于完全不想在目标群组留下日志记录的初次用户来说，这个分支应当是一个好的选择<br>
> 不过当然，如果你已经加入了原作者的群组，这个分支不会代行你进行退出<br>
> <br>**尊重个人选择，不行越权之事**<br><br>
> 本人理解大众对于这个分支的性质担忧，所以并未将该分支设为默认分支，且不计划进行公开发布<br>
> 如有需求，请自行进行构建，这符合编码能力要求的定义<br>~~（毕竟你都会自己构建了，禁掉这个功能也不是什么难事）~~
